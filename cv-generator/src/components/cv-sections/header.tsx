import { Globe, Link, Linkedin, Mail, Pin } from "lucide-react";
...
export const Header: React.FC<HeaderProps> = ({ fullName, title, email, location, github, linkedin, website }) => {
  return (
    <section className="flex flex-col justify-center items-center gap-1 mt-2 text-sm">
      ...
      {(github || linkedin || website) && (
        <div className="flex flex-wrap gap-2 justify-center items-center text-sm">
          {github && (
            <a className="inline-flex items-center gap-1" href={github.startsWith('http') ? github : `https://github.com/${github}`}>
              <Link className="w-3 text-black" aria-hidden="true" focusable="false" />
              <span>{github.replace('https://github.com/', 'github.com/')}</span>
            </a>
          )}
          {linkedin && (
            <a className="inline-flex items-center gap-1" href={linkedin.startsWith('http') ? linkedin : `https://linkedin.com/in/${linkedin}`}>
          ...
          {website && (
            <a className="inline-flex items-center gap-1" href={website.startsWith('http') ? website : `https://${website}`}>
